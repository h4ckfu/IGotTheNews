...
logging.basicConfig(filename='alt_app.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s')

sources = ['mashable', 'cleveland', 'religious', 'sbnation', 'huffpo', 'atlantic', 'telegram']
...
try:
    ...
    for source in sources:
        ...
        # variable for keyword news source text file
        file_name = 'data/keyword_list-' + source + '_alt.data'
        
        # Make the keyword list
        list_name = source + '_list'

        #Get the data from the source keyword file  
        with open(file_name, 'r') as f:
            source_text = ast.literal_eval(f.read())

        #Start making the DataFrame from individual source - df
        df = pd.DataFrame({'term':source_text})
        df['freq'] = df.groupby('term')['term'].transform('count')
        df = df.drop_duplicates()
        df = df.sort_values('freq',ascending = False)

        # merge the individual df in main DataFrame = ndf
    ...
    # Save mereged DataFrame to CSV file named as date
    archive_date = str(strftime("%Y-%m-%d", gmtime())) + '_alt.csv'
